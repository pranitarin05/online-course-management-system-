(function(){
  ...
  function getUrlParam(name){const u = new URLSearchParams(window.location.search); return u.get(name)}
  ...
  async function loadCourseDetail(){
    if(!courseId){
      qs('#courseDetail').innerHTML='<p>No course selected.</p>';
      return;
    }
    const res = await authorizedFetch(`${API_BASE}/courses/${courseId}/`);
    if(!res.ok){
      qs('#courseDetail').innerHTML='<p>Course not found.</p>';
      return;
    }
    ...
    detailDiv.innerHTML=`
      <img src="/assets/cover-sample.jpg" alt="${course.title}" style="width:100%;height:300px;object-fit:cover;border-radius:12px;margin-bottom:1rem;">
      <div>
        <h1>${course.title}</h1>
  ...
  reviewForm?.addEventListener('submit', async (e)=>{
    ...
    if(rating<1 || rating>5 || text.length<10){
      reviewMsg.textContent='Please fill all fields correctly';
      return;
    }
    ...
    try{
      ...
      if(res.ok){
        ...
        reviewForm.reset();
        loadCourseReviews();
      }else reviewMsg.textContent='Failed to post';
    }catch(e){reviewMsg.textContent='Error'}
  });
